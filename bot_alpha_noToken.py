...
AUSWAHL, HÖHE, WISSEN, NACHRICHT, SCHREIBEN, ENDE = range(6)

forderung = 0
# Hauptmenü des Bots
def start(bot, update):
    reply_keyboard = [['Rücklastschriftgebühren', 'Miete', 'Mit Dir']]
...
def gebühren(bot, update):
    update.message.reply_text('Wie viel Gebühren sollst du wegen der Rücklastschrift bezahlen?')
    return HÖHE
...
def zu_hoch(bot, update):
    ...
    if is_number(forderung):
        ...
        if 4 < forderung:
            ...
            ' War die Lastschrift denn vorher durch eine Rechnung angekündigt oder zieht die Firma regelmäßig Geld ein (z.B. Netflix-Abo)?',
            reply_markup=ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True))
            return WISSEN
        else:
...
def einsparung(bot, update):
    ...
    if antwort == 'Brief':
        zulässige_gebühren = 4
    elif antwort == 'SMS':
        zulässige_gebühren = 3.09
    elif antwort == 'E-Mail':
        zulässige_gebühren = 3
    elif antwort == 'Garnicht':
        zulässige_gebühren = 3
    gespart = forderung - zulässige_gebühren
    update.message.reply_text('Glückwunsch! Du sollstest %02d EUR bezahlen, allerdings sind in deinem Fall nur %02d EUR zulässig. Du kannst also %02d EUR sparen! Soll ich dir ein Musterschreiben generieren?' % (forderung, zulässige_gebühren, gespart), reply_markup=ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True))
    return SCHREIBEN

# Ausgabe des Schreibens an den Gläubiger
def print_schreiben (bot, update):
    update.message.reply_text("Die pauschale Absetzung von Rücklastschriftgebühren...")
    ende(bot, update)
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            AUSWAHL: [RegexHandler('^Rücklastschriftgebühren$', gebühren),
                    RegexHandler('^Miete$', miete),
                    RegexHandler('^Mit Dir$', gangsta)],

            HÖHE: [MessageHandler(Filters.text, zu_hoch)],
