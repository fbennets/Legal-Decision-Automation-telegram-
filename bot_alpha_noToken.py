...
def einsparung(bot, update):
    ...
    if antwort == 'Brief':
        zulaessige_gebueren = 4
    elif antwort == 'SMS':
        zulaessige_gebueren = 3.09
    elif antwort == 'E-Mail':
        zulaessige_gebueren = 3
    elif antwort == 'Garnicht':
        zulaessige_gebueren = 3
    gespart = forderung - zulaessige_gebueren
    update.message.reply_text('Glückwunsch! Du sollstest %02d EUR bezahlen, allerdings sind in deinem Fall nur %02d EUR zulässig. Du kannst also %02d EUR sparen! Soll ich dir ein Musterschreiben generieren?' % (forderung, zulaessige_gebueren, gespart), reply_markup=ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True))
    return SCHREIBEN
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            AUSWAHL: [RegexHandler('^Rücklastschriftgebühren$', gebueren),
